...
# Extract the version below <artifactId>auth-service</artifactId>
CURRENT_VERSION=$(awk '
...
if [[ -z "$CURRENT_VERSION" ]]; then
    echo "Error: Unable to find <version> tag for <artifactId>auth-service</artifactId> in pom.xml"
    exit 1
fi
...
# Update only the version below <artifactId>auth-service</artifactId>
